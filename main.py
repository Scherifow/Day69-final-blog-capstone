...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        if User.query.filter_by(email=email).first():
            flash('You already register this email')
    ...
    return render_template("register.html", form=register_form, logged_in=True)
...
def login():
    ...
    if request.method == "POST":
        ...
        if not user:
            flash("You wrote wrong email")
            return redirect(url_for('login'))
        elif not check_password_hash(user.password, password):
            flash("You wrote wrong password")
            return redirect(url_for('login'))
